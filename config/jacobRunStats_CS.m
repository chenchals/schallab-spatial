...
for ii = 1:numel(nhps)
    ...
    cSizes = cellfun(@(x) [ZZ.(nhp).(char(x)).contra_targetOnset.observed.cSize], sessions,'UniformOutput',false);
    cDists = cellfun(@(x) [ZZ.(nhp).(char(x)).contra_targetOnset.observed.dtnc], sessions,'UniformOutput',false);
    cNums = cellfun(@(x) cellfun(@length,{ZZ.(nhp).(x).contra_targetOnset.observed.cSize}),sessions,'UniformOutput',false);    
...
writetable(sizeStats,'ClusterStats_alignOnTargetOnset_observed.xlsx','Sheet','observed_clust_size','WriteRowNames',true,'WriteVariableNames',true)
writetable(distStats,'ClusterStats_alignOnTargetOnset_observed.xlsx','Sheet','observed_clust_dist','WriteRowNames',true,'WriteVariableNames',true)
writetable(numStats,'ClusterStats_alignOnTargetOnset_observed.xlsx','Sheet','observed_clust_num','WriteRowNames',true,'WriteVariableNames',true)
