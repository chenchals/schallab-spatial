...
%   Detailed explanation goes here
    ...
    channelTicks = 3.5:4:maxChannels;% offset by 0.5
    ...
    depthTickLabels = arrayfun(@(x) [num2str(x) ' \mum'],(channelTicks+0.5).*channelSpacing,'UniformOutput',false);
     xstep = 0.1; 
    set(plAxis(1),'YTick',channelTicks,'YTickLabel',2:2:numel(channelMap),'YColor','k', 'FontSize', 16);    
    set(plAxis(2),'YTick',channelTicks,'YTickLabel',{' '},'YColor','k', 'FontSize', 16);
...
%      xstep = 0.1; 
%     set(plAxis(1),'YTick',channelTicks,'YTickLabel',2:2:numel(channelMap),'YColor','k', 'FontSize', 16);    
%     set(plAxis(2),'YTick',channelTicks,'YTickLabel',depthTickLabels,'YColor','k', 'FontSize', 16);
%     set(plAxis,'YDir','reverse','XTick',[],'TickDir','both','box','on');
%     set(plAxis,'YLim',[0 maxChannels],'XLim',[probeLoc-xstep probeLoc+xstep]);
%     %hold on
%     set(figH,'currentAxes', plAxis(1));
...
%     patch('XData',xData,'YData',yData','FaceColor',[0.8,0.8,0.8],'FaceAlpha',0.4);
%     currAxes.Clipping ='off';   
%     ylabel('Channels','FontWeight','bold', 'FontSize',28);
% 
% end
