...
% 'sdf Mean Ztr'
xValsToUseArr = {'R^2', 'sdf Mean FR', 'sdf Mean FR (Z)','sdf Mean Ztr'};

    for xx = 1: numel(xValsToUseArr)
        xValsToUse = xValsToUseArr{xx};
        ...
        plotMoransFig(sess, mi, dists, xValsToUse)
    end
end
...
function [] = plotMoran(neighborDistance, distance, moranI, alpha)
    gMoran = moranI(1);
    gAlphaMoran = alpha(1);
    lMoran = moranI(2:end);
    lAlphaMoran =  alpha(2:end);
    ...
    title(titleTxt,'Interpreter','tex');
    [~, yTicks, yTickLabels] = getTicks(distance);
    set(gca,'YTick', yTicks,'YTickLabel',yTickLabels,'YGrid','on','YMinorGrid','on')
    set(gca,'YDir','reverse')
    set(gca,'YLim',[0 max(distance)])
    set(gca,'XLim',xLims)
