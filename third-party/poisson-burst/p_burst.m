...
function [BOB, EOB, SOB]=p_burst(InTrain, StartT, StopT, varargin)
...
if ~isempty(varargin)
    MinSPInBurst=varargin{1};
else
    MinSPInBurst=4;%Minimum spkes in a Burst
end
...
while(FspAB <= MaxSpikes-1 | ~Done)
   ...
   while(Iterate)
      ...
      Prob=(poisscdf([1:length(cISI)]', double(cISI.*MU)))+Tol;
   ...
   if(ISBURST)
      %CHECK CRITERIA
      MaxSI=-log(1-(poisscdf(CurrEOB-CurrBOB,double(SPT(CurrEOB)-SPT(CurrBOB))*MU)));
